(function($, jasmine) {
  ...
  function findMatchingOptions(realOptions, allOtherOptions) {
    ...
    var rules = [
      {field: 'url', comparator: matchesRegex},
      {field: 'type', comparator: _.isEqual},
      {field: 'data', comparator: _.isEqual},
      {field: 'dataType', comparator: _.isEqual},
      {field: 'async', comparator: _.isEqual}
    ]
    var allMatchingOptions = _.select(allOtherOptions, function(otherOptions) {
      ...
      return _.all(rules, function(rule) {
        return _.isUndefined(other[rule.field]) ? true : rule.comparator(real[rule.field], other[rule.field])
      })
    })
